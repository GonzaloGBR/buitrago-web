...
import { products, getProduct, getCategory, getProductsByCategory } from "@/data/products";
...
export default function ProductPage({
  ...
  const similarProducts = getProductsByCategory(params.slug).filter(
    (p) => p.id !== product.id
  );
    ...
    <main className="min-h-screen bg-cream">
      ...
      <div className="section-editorial pt-6 pb-2">
        ...
        <nav className="mt-3 font-sans text-[0.7rem] text-warm-gray">
          ...
          <span className="mx-1.5">/</span>
          <Link href={`/categoria/${params.slug}`} className="no-underline hover:text-charcoal transition-colors">{category.name}</Link>
          <span className="mx-1.5">/</span>
        ...
        <section className="section-editorial border-t border-sand/30 pb-20 pt-14 md:pb-28">
          ...
          <div className="grid grid-cols-2 gap-4 md:grid-cols-3 md:gap-7">
            {similarProducts.slice(0, 3).map((p) => (
              <Link
                key={p.id}
                href={`/categoria/${params.slug}/${p.id}`}
